...
public class Font {
	...
	public void renderText(String text, int x, int y) {
		...
		for (int i = 0; i < text.length(); i++) {
			...
			s.setWidth(size);
			s.setHeight(size);
			Main.getScreen().renderSpriteIgnoringCamera(fontImg + " - " + text.charAt(i), new Vector2f(x + (i * (size + spacing)) - ((text.length() * (size + spacing)) / 2), y - (size / 2)), new float[] { 0.0f, 1.0f, 0.0f, 1.0f }, new float[] { colour.getRed() / 255.0f, colour.getGreen() / 255.0f, colour.getBlue() / 255.0f, colour.getAlpha() / 255.0f });
			s.setWidth(prevSize);
